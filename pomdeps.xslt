...
<xsl:stylesheet version='1.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'>
	...
	<xsl:template match='dependency'>
		...
		<xsl:text/><xsl:value-of select='groupId'/>:<xsl:value-of select='artifactId'/>:<xsl:value-of select='version'/>:<xsl:value-of select='$type'/>:<xsl:value-of select='$scope'/><xsl:text>&#xA;</xsl:text>
	</xsl:template>
