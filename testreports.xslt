...
<xsl:stylesheet version='1.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'>
	...
	<xsl:template match='/'>
		...
		<xsl:text/>Summary: tests run <xsl:value-of select="sum(//failures/runcount) + sum(//success/runcount)"/>
		<xsl:text/> tests failed <xsl:value-of select="sum(//failures/failcount)"/> 
		<xsl:text/> errors <xsl:value-of select="sum(//failures/errorcount)"/>
		<xsl:text>&#xA;</xsl:text>
	</xsl:template>

	<xsl:template match='testcase'>
		<xsl:text/>Test case: <xsl:value-of select='name'/> runtime <xsl:value-of select='runtime'/><xsl:text>&#xA;</xsl:text>
	...
	<xsl:template match='defects[@type="failure"]'>
